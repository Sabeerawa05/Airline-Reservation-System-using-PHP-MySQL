...
if ((isset($_POST['scheduid'])) && ($_POST['scheduid'] != "")) {
  $deleteSQL = sprintf("DELETE FROM schedule WHERE schedulID=%s",
                       GetSQLValueString($_POST['scheduid'], "int"));

  mysql_select_db($database_connection1, $connection1);
  $Result1 = mysql_query($deleteSQL, $connection1) or die(mysql_error());

  $deleteGoTo = "admindisplayschedule.php";
...
if ((isset($_POST['schedulid'])) && ($_POST['schedulid'] != "")) {
  $deleteSQL = sprintf("DELETE FROM schedule WHERE schedulID=%s",
                       GetSQLValueString($_POST['schedulid'], "int"));
...
if (isset($_POST['schedulid'])) {
  $colname_delete = (get_magic_quotes_gpc()) ? $_POST['schedulid'] : addslashes($_POST['schedulid']);
}
mysql_select_db($database_connection1, $connection1);
$query_delete = sprintf("SELECT * FROM schedule WHERE schedulID = %s", $colname_delete);
$delete = mysql_query($query_delete, $connection1) or die(mysql_error());
...
<title>admin delete scheduled</title>
<style type="text/css">
<!--
body {
...
Schedule to Delete:
  <label>
  <input name="scheduid" type="text" id="scheduid" />
  </label>
  <label>
